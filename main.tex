...
No single tool addresses all dimensions of reproducibility, but many tools contribute to multiple STAMPED properties simultaneously. For example, container technologies such as Docker and Singularity/Apptainer serve Portability by providing OS-level isolation from explicit environment specifications, Ephemerality by enabling disposable per-execution environments, and Distributability by producing frozen, shareable artifacts.
Similarly, version control systems like Git underpin both Tracking (content-addressed identification and change history) and Self-containment (gathering all components under a single root), while extensions such as git-annex, DataLad, DVC, and Git LFS further extend these properties to large and binary data.

This overlap is not incidental — it reflects the interdependence of the STAMPED properties themselves. Achieving Ephemerality, for instance, inherently exercises Portability and Actionability: a disposable environment must be reconstituted from explicit specifications, and its setup must be automated end-to-end.
...
A representative example is the choice between \texttt{git-annex} and Git LFS for large-file tracking---both appear under Self-containment and Tracking in Table~\ref{tab:stamped-tools}, but they occupy different points on a complexity--flexibility spectrum.
\texttt{git-annex} supports many remote types (SSH, S3, HTTP, local drives, offline USB archives) and integrates with DataLad's federated subdataset model, making it well-suited to multi-institutional collaboration and long-term archival where data sovereignty or offline access is required.
Git LFS offers a simpler alternative: it is transparent to users, natively supported by GitHub and GitLab, and broadly adopted---but it is centralized (requiring an LFS server), lacks offline capability, and provides less flexible remote configurations.
...
Make, Snakemake, Nextflow, CWL, \texttt{datalad run}, shell scripts &
Define executable specifications for workflows; provide clear entry points for reproducing results \\
\hline
\textbf{M — Modularity} &
\texttt{Git} submodules, DataLad subdatasets, Kedro, Cookiecutter Data Science &
...
\hline
\textbf{P — Portability} &
Docker, Singularity/Apptainer, Nix, Guix, conda, \texttt{pip} (\texttt{pyproject.toml}, \texttt{requirements.txt}) &
...
Docker, Singularity/Apptainer, \texttt{docker compose}, Slurm, cloud CI/CD (e.g., GitHub Actions) &
...
Zenodo, DANDI, PyPI, conda-forge, Docker Hub/registries, DataLad, RO-Crate, \texttt{npm ci}, \texttt{pip freeze} &
