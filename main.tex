...
In many cases, users interact with models through platforms without knowing which specific model version --- or even which model --- produced their results.
...
Practical mechanisms exist for open models, such as pinned revisions on Hugging Face or cached weights stored via git-annex, and models managed in this way would immediately offer greater value for use in scientific workflows.
...
When an agent can regenerate analysis code from a method specification, the specific implementation becomes disposable --- robustness resides in the specification, not in any particular codebase.
